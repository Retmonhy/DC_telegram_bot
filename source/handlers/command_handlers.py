...
Есть две стратегии рассчета: 
- задать количество бутылок, которое пользователь хочет потратить и посчитать сколько изумрудов получится;
- задать желаемое количество изумрудов и посчитать сколько бутылок и времени потребуется для этого. 
Так же в результатах рассчета можно будет увидеть затраты по времени на процесс ТТ.
Уже произведено <b>{ModelStore.total_calculations} расчетов</b>
